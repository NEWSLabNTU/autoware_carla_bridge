...
1. **Waits for Autoware Instance** (instead of pre-existing CARLA vehicle)
   - Subscribes to `/robot_description` topic (URDF with sensor definitions)
   - Detects Autoware instance when robot_description is received
   - Handles Autoware appearance and disappearance

2. **Parses Sensor Configuration from URDF**
   - Extracts sensor types, transforms, and parameters from URDF
   - Maps URDF sensor definitions to CARLA sensor blueprints
   - No more hardcoded sensor configurations

3. **Subscribes to Initial Pose**
   - Waits for `/initialpose` (from RViz 2D Pose Estimate)
   - Determines where to spawn vehicle in CARLA world

4. **Spawns CARLA Vehicle When Ready**
   - Spawns vehicle only when: Autoware detected + initial pose received + sensor config parsed
   - Attaches sensors to vehicle based on URDF transforms
   - Vehicle lifecycle tied to Autoware lifecycle
...
5. **Testing Workflow**:
   ...
   # Step 2: Start bridge (waits for Autoware)
   ros2 run autoware_carla_bridge autoware_carla_bridge

   # Step 3: Start Autoware planning simulator
   cd ~/autoware && ros2 launch autoware_launch planning_simulator.launch.xml

   # Step 4: Set initial pose in RViz (2D Pose Estimate)
   # -> Vehicle spawns in CARLA at specified location

   # Step 5: Compare CARLA and Autoware topics
   ros2 topic echo /sensing/lidar/top/pointcloud
   ```

**Benefits**:
- ✅ Bridge adapts to any Autoware sensor configuration (no hardcoding)
- ✅ No manual sensor setup required
