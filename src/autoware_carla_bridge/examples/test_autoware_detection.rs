...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("Starting Autoware detection test...");
    ...
    tracing::info!("Creating AutowareDetector...");
    ...
    tracing::info!("Waiting for Autoware detection (timeout: 60s)...");
    tracing::info!("Spinning executor to process ROS messages...");
    ...
    if let Some(urdf) = detector.get_urdf() {
        tracing::info!("URDF received ({} bytes)", urdf.len());
        ...
        match parse_urdf_sensors(&urdf) {
            ...
            Err(e) => {
                tracing::error!("Failed to parse URDF: {:?}", e);
            }
        }
    } else {
        tracing::warn!("No URDF available yet");
    }

    // Check additional components
    tracing::info!("Checking robot_state_publisher...");
    match detector.check_robot_state_publisher() {
        Ok(found) => {
            if found {
                tracing::info!("  ✓ robot_state_publisher node found");
            } else {
                tracing::warn!("  ✗ robot_state_publisher node not found");
            }
        }
        Err(e) => {
            tracing::error!("  Error checking robot_state_publisher: {:?}", e);
        }
    }

    tracing::info!("Checking /tf_static topic...");
    match detector.check_tf_static_topic() {
        Ok(found) => {
            if found {
                tracing::info!("  ✓ /tf_static topic found");
            } else {
                tracing::warn!("  ✗ /tf_static topic not found");
            }
        }
        Err(e) => {
            tracing::error!("  Error checking /tf_static topic: {:?}", e);
        }
    }
    ...
    tracing::info!("  State: {:?}", diag.state);
    tracing::info!("  URDF available: {}", diag.urdf_available);
    if let Some(size) = diag.urdf_size {
        tracing::info!("  URDF size: {} bytes", size);
    }
    if let Some(elapsed) = diag.time_since_last_update {
        tracing::info!("  Time since last update: {:?}", elapsed);
    }
    ...
    match TFBuffer::new(node.clone()) {
        Ok(tf_buffer) => {
            tracing::info!("TF Buffer created successfully");

            // Give it a moment to receive transforms
            std::thread::sleep(Duration::from_secs(2));

            tracing::info!("TF Buffer has {} transforms", tf_buffer.len());

            if !tf_buffer.is_empty() {
                tracing::info!("Available frames:");
                ...
                if frames.len() >= 2 {
                    ...
                    match tf_buffer.lookup_transform(target, source) {
                        ...
                        Err(e) => {
                            tracing::warn!("  Could not lookup transform: {:?}", e);
                        }
                    }
                }
            } else {
                tracing::warn!("No transforms received yet");
            }
        }
        Err(e) => {
            tracing::error!("Failed to create TF Buffer: {:?}", e);
        }
    }
    ...
    tracing::info!("Position conversion:");
